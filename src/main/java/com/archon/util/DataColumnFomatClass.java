...
自己研究的，效率低，待改进。未使用！
*/
public class DataColumnFomatClass {

    private String packageOutPath = "main.java.com.archon.po";//指定实体生成所在包的路径
    private String packageClassPath = "com.archon.po";
    private String javaName =null; //.java文件名
    private String authorName = "shen";//作者名字
    private String tablename = javaName;//表名
    private String[] colnames; // 列名数组
    private String[] colTypes; //列名类型数组
    private int[] colScales;//列名精度数组
    ...
    private static final String NAME = "archon";
     ...
     * 构造函数
     */
    public DataColumnFomatClass(String javaName){
...
//          }
    ...
    private String parse(String[] colnames, String[] colTypes, int[] colSizes) {
        ...
        //导入包名
        sb.append("package " + this.packageClassPath + ";\r\n");
        //判断是否导入工具包
        if(f_util){
    ...
    private void processAllAttrs(StringBuffer sb) {
        ...
        for (int i = 0; i < colnames.length; i++) {
            sb.append("\tprivate " + sqlType2JavaType(colTypes[i],colScales[i]) + " " + colnames[i].toLowerCase() + ";\r\n");
        }
        //换行
        sb.append("\r\n");
    }
    ...
    private void processAllConstructor(StringBuffer sb){
        ...
        for (int i = 0; i < colnames.length; i++) {
            if(i == colnames.length - 1){
                sb.append(sqlType2JavaType(colTypes[i].toLowerCase(),colScales[i]) + " " + colnames[i].toLowerCase() + "){\r\n");
            }else{
                sb.append(sqlType2JavaType(colTypes[i].toLowerCase(),colScales[i]) + " "  + colnames[i].toLowerCase() + ", ");
            }
        }
     ...
     * @return
     */
    private String sqlType2JavaType(String sqlType,int sqlScale) {
        if(sqlScale==0){
